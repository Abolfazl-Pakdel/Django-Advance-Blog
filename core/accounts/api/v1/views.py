...
class RegistrationApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, to=[email])
...
class TestEmailSend(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        self.email = 'masoud@gmail.com'
        user_obj = get_object_or_404(User, email=self.email)
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage('email/hello.tpl',{'token':token},'admin@admin.com',to=['Abolfazlpakdelpour@gmail.com'])
        EmailThread(email_obj).start()
        # send_mail('email/hello.tpl',{'name':'Abolfazl'},'admin@admin.com',['Abolfazlpakdelpour@gmail.com'])
